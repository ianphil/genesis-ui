...
- In chamber, run `npm run lint` and `npm test` (vitest). SDK-touching changes also run `npm run test:sdk-smoke`. Packaging-touching changes run `npm run make:sandbox`.
